export class CurrentWidget {
  ...
  constructor() {
    ...
    this.#cityElement.className = "city-name";
    this.#htmlElement.appendChild(this.#cityElement);
  ...
  async update(data) {
    ...
    this.#cityElement.textContent = data[0].name;
    ...
    const timezoneAsHHMM =
      (positive ? "+" : "-") + String(timezoneHours) + String(timezoneMinutes);
    const options = { timeZone: timezoneAsHHMM };
    this.#timeElement.textContent = date.toLocaleTimeString([], {
      timeZone: timezoneAsHHMM,
      hour: "2-digit",
      minute: "2-digit",
      hour12: false,
    });
